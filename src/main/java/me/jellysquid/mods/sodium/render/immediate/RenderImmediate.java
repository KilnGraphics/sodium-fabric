...
public class RenderImmediate {
    ...
    private final Map<SequenceBuilder, SequenceIndexBuffer> defaultElementBuffers;
...
//        for (SequenceBuilders sequenceType : SequenceBuilders.values()) {
...
//        }
    ...
    public void draw(Pipeline<VanillaShaderInterface, VanillaShaderInterface.BufferTarget> pipeline, ShaderTexture[] shaderTextures,
        ...
        if (useDefaultElementBuffer) {
            var sequenceBufferBuilder = this.defaultElementBuffers.computeIfAbsent(SequenceBuilders.map(drawMode, IntType.UNSIGNED_INT), sb -> new SequenceIndexBuffer(device, sb));
